...
echo 'subscription_plan_module: '.(\Schema::hasTable('subscription_plan_module') ? 'YES' : 'NO').PHP_EOL;
...
foreach (\$modules as \$module) {
    \$planCount = DB::table('subscription_plan_module')
...
foreach (\$plans as \$plan) {
    \$count = DB::table('subscription_plan_module')
